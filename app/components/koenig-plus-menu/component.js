...
import CARD_MENU from '../text-editor/cards';
...
export default Component.extend({
    ...
    init() {
        ...
        this.itemSections = CARD_MENU.reduce((sections, section) => {
          let items = section.items.filter(item => this.embeds.indexOf(item.replaceArg) !== -1);
          if (items.length > 0) {
              sections.push({
                  title: section.title,
                  items
